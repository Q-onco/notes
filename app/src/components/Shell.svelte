<script lang="ts">
  ...
  const MORE_NAV = [
    { id: 'journal',       label: 'Journal',       icon: 'M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z' },
    { id: 'calendar',      label: 'Calendar',      icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
    { id: 'audio',         label: 'Audio',         icon: 'M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4M12 3a7 7 0 017 7' },
    { id: 'pipeline',      label: 'Pipeline',      icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
    { id: 'files',         label: 'Files',         icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z' },
    { id: 'grants',        label: 'Grants',        icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    { id: 'manuscript',    label: 'Manuscript',    icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
    { id: 'presentations', label: 'Slides',        icon: 'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z' },
...
<div class="shell">
  ...
  {#if moreOpen}
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div class="more-backdrop" onclick={() => moreOpen = false}></div>
    <div class="more-drawer" role="dialog" aria-label="More sections">
      <div class="more-grid">
        {#each MORE_NAV as item}
          <button
            class="more-item"
            class:active={store.view === item.id}
            onclick={() => { store.view = item.id as any; moreOpen = false; store.sidebarOpen = false; }}
  ...
  <nav class="bottom-nav" aria-label="Main navigation">
    {#each BOTTOM_NAV as item}
      <button
        class="bn-item"
        class:active={store.view === item.id}
        onclick={() => { store.view = item.id as any; store.sidebarOpen = false; moreOpen = false; }}
        aria-label={item.label}
      >
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
          <path d={item.icon}/>
    ...
    <button
      ...
      onclick={() => { moreOpen = !moreOpen; store.sidebarOpen = false; store.enzoOpen = false; }}
    ...
    <button
      ...
      onclick={() => { store.enzoOpen = !store.enzoOpen; store.sidebarOpen = false; moreOpen = false; }}
      aria-label="Enzo AI"
    >
...
<style>
  ...
  @media (max-width: 640px) {
    ...
    .more-item:active, .more-item.active { background: var(--ac-bg); color: var(--ac); }
    .more-item.active svg { stroke: var(--ac); }
    .more-item-label { font-size: 0.6rem; font-weight: 600; letter-spacing: 0.01em; }
  }
