<script lang="ts">
  ...
  const NEW_FEATURES = [
    { color: 'ac', icon: 'M6 5c7.18 0 13 5.82 13 13M6 11a7 7 0 017 7M6 17a1 1 0 110-2 1 1 0 010 2z', title: 'Jobs: Live Academic Feed + Topic Search', desc: 'Type any research topic (e.g. "HGSOC spatial transcriptomics", "ovarian cancer postdoc Heidelberg", "scRNA-seq bioinformatics") in the Feed topic bar and press Enter. Live listings are pulled in parallel from Nature Careers, EurAxess, jobs.ac.uk, and Academic Positions EU — up to 40 results, deduped and sorted newest-first. Filter by Region and Type locally. Previously the feed was locked to "oncology"; now any topic works.' },
    { color: 'bl', icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253', title: 'Bibliography: Reference Manager + PDF + Network + AI', desc: 'A complete Zotero/Mendeley replacement. Import by DOI, PMID, arXiv, BibTeX/RIS, or manual. Four citation styles. 80 curated landmark papers across 10 oncology categories. Cross-module: save from Research, cite in Manuscript, attach to Pipeline steps. PDF Viewer: open OA PDFs in-app, highlight text, annotate with notes, export to Notes; PDF full-text search scans all pages and jumps between matches. Citation Network (Cytoscape): auto-fetch citation links via OpenCitations (free), 4 layouts, export PNG. AI Analysis (Enzo): per-paper Summarise / Key Quotes / Gaps / PICO / Auto-tag; multi-paper Compare / Synthesis / Gap Analysis; annotations are automatically included as AI context. Reading Queue sidebar shows unread/in-progress PDFs with page-resume. Notes↔Biblio: cite library papers directly in the Editor, and see which notes cite a paper in the detail panel.' },
    { color: 'gn', icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', title: 'Launchpad: Funding Hub + In-app Media', desc: 'New Funding tab with 16 curated government grants (US/EU/Germany/India/UK/Switzerland), 21 VC & angel investors across all geographies including India-specific funds (Aarin, Chiratae, Eight Roads, Villgro), live funding news with in-app reader, and Enzo Funding Advisor (70B, India + EU context). Podcast episodes now play directly inside the app — RSS fetch → episode list → HTML5 player with progress bar and speed control. Video/playlist resources open a built-in YouTube iframe panel. Mini player bar persists at the bottom while audio plays.' },
    { color: 'pu', icon: 'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75M9 7a4 4 0 100 8 4 4 0 000-8z', title: 'Audio: Lecture Mode', desc: 'Switch to Lecture Mode before recording to capture a named session with venue. Each 15-second Whisper chunk is instantly classified as Lecturer or Audience using heuristics — question patterns, sentence length, opening phrases. After stopping, hit "Identify Speakers ✦" to run a Groq 70B pass for a more accurate re-labelling. Saves with full speaker turns so the note export has structured Lecturer / Audience blocks.' },
    { color: 'ac', icon: 'M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z', title: 'Launchpad: Biotech Founder Hub', desc: 'A curated startup library with 60+ resources — podcasts, video channels, playlists, communities, tools, and newsletters — all filterable by type and stage. Step-by-step SOPs for Lab→Company, IP/TTO, entity formation, SBIR, SAB, pitch decks, and FDA. Live biotech news from 4 RSS feeds. Bookmark anything, add custom resources, and ask Enzo Founder GPS to guide you through any startup challenge.' },
    { color: 'ac', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', title: 'Research: Automatic Search History', desc: 'Every search you run is now logged automatically in the "Saved" tab under History. Each entry shows the query, sources used, result count, and timestamp. Re-run any past search in one click, bookmark it to keep it pinned, or delete individual entries. Clear all history from the header row.' },
    ...
    { color: 'enzo', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', title: 'Research: Knowledge Graph Paper Compare', desc: 'Select 2–3 papers using the checkboxes, then click "Compare with Enzo". Enzo builds an interactive force-directed knowledge graph — coloured nodes for methods, genes, findings, pathways, and biomarkers; edges with typed relations (employs/validates/contradicts/extends…) and weights. Drag nodes, scroll to zoom, pan by dragging the canvas. Use the Density slider to control how many concept nodes are shown (8–30). Click any concept node to open a sliding panel with a 250-word Enzo analysis of that concept in the context of your papers. After the graph loads, a 500-word Deep Analysis streams below — covering methodological differences, finding agreements and contradictions, and translational significance for HGSOC. Maximise the modal to full screen. Save the comparison as a note.' },
    ...
    // ── 2026-05-26 Deploy A/B/C ──
    { color: 'ac',  icon: 'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z', title: 'Jobs: JobVector + Euraxess + AI Match Score', desc: 'Job feed now pulls from six additional sources: JobVector.de (3 feeds — general science, postdoc, Heidelberg-area) and Euraxess (DE, global, CH). Each card shows a colour-coded source badge. Filter by source with the chips row. New AI Match Score button per card: Enzo compares the job description against your CV profile and scores the fit 0–100 with a 2-sentence rationale shown inline. Scores are cached so re-clicking is instant. One-click "Save + Letter" saves the job and opens the cover letter tab pre-filled.' },
    { color: 'gn',  icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01', title: 'Lab Tools: Dilution Suite, Experiment Builder, Stats, Troubleshoot, Reagents', desc: 'New sidebar tab with five panels. Dilutions: serial dilution calculator with SVG tube-chain diagram, C₁V₁=C₂V₂ solver (solve any unknown), and cell-seeding calculator with viability input. Experiments: 8-type protocol builder with HGSOC-specific control checklists (PARPi, flow cytometry, scRNA-seq, etc.), biological/technical replicate input, power analysis (effect size → n per group), export to note. Stats Advisor: 3-question guided flow (groups → paired/unpaired → normal/non-normal) → recommended test + assumptions + Enzo explanation on demand. Troubleshoot: describe your problem, Enzo streams a ranked differential diagnosis with mechanisms and fixes. Reagent Search: PubChem lookup (free, no key) — CAS, MW, formula, InChI; Enzo Most-cited vendor/catalog# button; insert reagent block to note.' },
    { color: 'enzo', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', title: 'Research: Visual Paper Compare — Radar Chart Modal', desc: 'Paper Compare now opens a full-screen modal with a structured SVG radar chart. Enzo scores each paper on 6 axes (Sample Size, Methodology, Novelty, HGSOC Relevance, Statistical Power, Translatability) on a 1–5 scale. Two overlapping polygons (blue P1, purple P2) animate in. A detailed comparison table sits alongside the chart with one-sentence per-paper descriptions per dimension. A verdict strip at the bottom summarises the key difference. Save the full analysis as a structured markdown note. Press Escape or click outside to close.' },
    ...
    // ── 2026-05-12 Graph relational strength + journal nodes ──
    { color: 'ac',  icon: 'M13 10V3L4 14h7v7l9-11h-7z', title: 'Note Graph: Relational Strength Edges', desc: 'The graph now computes pairwise relational strength between every note and journal entry using four signals: TF-IDF cosine content similarity, tag Jaccard overlap, explicit [[link]] count, and co-citation score. Composite edges are drawn with thickness and opacity proportional to strength. Edge colour indicates dominant signal: blue = links, green = tags, purple = content. Hover any edge to see a percentage breakdown. Use the Threshold slider in the toolbar to hide weaker edges.' },
    { color: 'enzo', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z', title: 'Note Graph: Bridge ⚡ & Spark ✧', desc: 'Select exactly 2 notes in multi-select mode and click Bridge ⚡ — Enzo (70B) writes a bridge research note connecting them, showing what each brings and what directions emerge. Spark ✧ detects algorithmically weak-but-meaningful pairs (high content similarity, zero explicit links) and generates an idea note with a hypothesis and first step. Both generate "Save as Note" directly into your notes.' },
    { color: 'yw',  icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', title: 'Note Graph: Journal Nodes (◇ Diamond)', desc: 'Journal entries now appear in the knowledge graph as amber diamond nodes — visually distinct from circular note nodes. Their strength edges are computed using mood/context tags and body content via the same TF-IDF pipeline. Double-click any journal diamond to open that day\'s journal. The Heatmap (Hub) mode colours every node from cool blue (isolated) to hot orange (high average connectivity).' },
    { color: 'ac',  icon: 'M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01', title: 'Notes & Journal: Font Colours', desc: 'The rich text toolbar now includes a 24-colour palette with reds, oranges, greens, blues, purples, and neutral tones. Enter any custom hex code in the colour picker and press ↵ to apply. A preview dot shows the colour before you apply it. "Remove colour" clears formatting back to default.' },
    { color: 'gn',  icon: 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z', title: 'Research: Search Autocomplete', desc: 'The Research search bar now shows a dropdown of your recent unique queries as you focus it. Click any past query to instantly re-run it. Grouped history with per-day and per-entry delete is available in the History sub-tab of the Saved section.' },
    // ── 2026-05-27 Systematic Review Conductor ──
    { color: 'ac',  icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01', title: 'Systematic Review Conductor', desc: 'Full PRISMA 2020-guided systematic review in 5 stages. Protocol: define PICO question, inclusion/exclusion criteria, and custom data extraction fields. Search: query PubMed and OpenAlex simultaneously, auto-deduplicate by DOI + normalised title. Screening: card-by-card title/abstract decisions with progress bar and filter pills; hit "Enzo screens all" to batch pre-screen (Llama-3.1-8B) — Enzo shows include/exclude/uncertain with a one-line reason you can accept or override. Full-text screening works identically on S1-included papers. Extraction: per-paper form with 11 HGSOC-relevant fields; Enzo pre-fills from the abstract in one click; add custom fields any time. Synthesis: live PRISMA 2020 SVG flow diagram, evidence table, TSV export, and Enzo-streamed Methods + Results drafts.' },
    // ── 2026-05-27 Grant Writing Studio ──
    { color: 'gn',  icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', title: 'Grant Writing Studio', desc: 'Full-cycle grant application environment for NIH R01/R21, ERC Starting Grant, DFG Sachbeihilfe, Wellcome Discovery, and custom calls. Five tabs: Aims (CRUD aims with hypothesis/approach/milestone; Enzo drafts the NIH Specific Aims 1-pager), Sections (agency-specific section templates, live word-count bars, Enzo draft + accept flow for each section), Budget (line-item table with per-year columns and auto-totals for 3 years), Review (add reviewer comments by criterion with optional score; Enzo drafts a point-by-point resubmission response per comment; run full mock study section critique with impact score and strengths/weaknesses), Preview (assembled full application text, total word count, copy-all). Opens with a pre-filled HGSOC R01 example: macrophage–CAF crosstalk and PARPi resistance in high-grade serous ovarian cancer.' },
  ];
  ...
  const ALL_MODULES = [
    ...
    { id: 'jobs',          label: 'Jobs & Career',   color: 'ac',   icon: 'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z', desc: 'Live academic job feed (Nature Careers, EurAxess, JobVector.de, Euraxess EU) with free-text topic search, source filter chips, and colour-coded source badges per card. AI Match Score: per-job Enzo scoring against your CV (0–100 + 2-sentence rationale, cached). One-click Save + Cover Letter. 7-stage pipeline tracker (Radar → Offer) with per-job interview log, notes, and next-action tracking. Curated company directory (EU pharma + India pharma/research). Full CV builder with export (PDF/DOCX/MD). Enzo cover letter generator with prior-letter memory. Contacts/network log. Salary benchmarking. Application analytics dashboard.' },
    { id: 'labtools',      label: 'Lab Tools',       color: 'gn',   icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z', desc: 'Five-panel lab toolkit. Dilutions: serial dilution with SVG tube-chain diagram, C₁V₁=C₂V₂ solver (solve any single unknown), cell-seeding calculator with viability correction. Experiments: 8-type protocol builder (Cell viability, Western blot, Flow cytometry, qPCR, IF/ICC, CRISPR, Co-IP, scRNA-seq) with HGSOC-specific control checklists, replicate input, and power analysis (effect size → n/group). Stats Advisor: 3-question guided decision tree → recommended test + assumptions + Enzo streaming explanation. Troubleshoot: describe your experiment + observed problem, Enzo streams ranked differential diagnosis with mechanisms and fixes. Reagent Search: PubChem lookup (no key needed) returning CAS, MW, IUPAC, InChI; Enzo Most-cited vendor/catalog# button; one-click insert to note.' },
    ...
    { id: 'sysreview',     label: 'Sys. Review',     color: 'ac',   icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01', desc: 'PRISMA 2020-guided systematic review conductor. 5 stages: Protocol (PICO + criteria + extraction fields), Search (PubMed + OpenAlex with auto-dedup), Screening S1 (title/abstract cards + Enzo batch screen), Screening S2 (full-text), Extraction (per-paper form with Enzo pre-fill), Synthesis (live PRISMA SVG + evidence table + TSV export + Enzo draft sections).' },
    { id: 'grantwriter',   label: 'Grant Studio',    color: 'gn',   icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', desc: 'Full-cycle grant application environment: aims builder, per-section Enzo drafting with word-count bars, 3-year budget table, reviewer response drafting with mock study section critique, and assembled preview. Supports NIH R01/R21, ERC Starting Grant, DFG Sachbeihilfe, Wellcome Discovery, and custom calls.' },
  ...
  const SHORTCUTS = [
    ...
    { group: 'Bibliography', items: [
      { key: 'PDF search bar → Enter', desc: 'Scan all PDF pages for your query; jumps to first matching page' },
      { key: '‹ / › (search results)', desc: 'Navigate between pages that contain the search term' },
      { key: 'Citation panel → type ≥ 2 chars', desc: 'Shows "My Library" matches above PubMed results; click to insert Author et al. (Year) as a link' },
      { key: '"Cited in N notes" (detail panel)', desc: 'Click any note title to jump directly to that note in the Notes module' },
      { key: 'Reading Queue (sidebar)', desc: 'Lists unread/in-progress refs with PDFs; click to open with page resume' },
      { key: 'Annotations → AI', desc: 'Existing highlights are automatically passed as context to all Enzo AI analyses' },
    ]},
    ...
    <div class="content">
      ...
      {:else if active === 'modules'}
        <div class="section-wrap">
          <div class="section-head">
            <h2>All features</h2>
            <p class="section-sub">16 sections. Every one encrypted at rest on GitHub.</p>
          </div>
...
<style>
  ...
  .feature-icon-bl   { background: rgba(99,102,241,.1); color: #818cf8; }
